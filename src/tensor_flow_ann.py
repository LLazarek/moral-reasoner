...
def forward_propogate(hidden_size, X, theta0, theta1):
    ...
    biases_output = tf.get_variable('biases_output', shape=[1], initializer=tf.constant_initializer(0.1), dtype=tf.float32)
    hidden_activations = tf.nn.sigmoid(tf.nn.bias_add(tf.matmul(X, theta0), biases_hidden))
    return tf.sigmoid(tf.nn.bias_add(tf.matmul(hidden_activations, theta1), biases_output))
...
def train(X_train, y_train, X_test, y_test):
    ...
    for iteration in range(100):
        ...
        train_predicted = sess.run(predict, feed_dict={X: X_train, y: y_train})
        train_expected = np.matrix(y_train) # Get index of largest value
        ...
        test_expected = np.matrix(y_test) # Get index of largest value
